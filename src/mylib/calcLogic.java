...
// ^=94  %=37 +=43 *=42 -=45 /=47 (->40  )->41 .=46
    private long bodmas(ArrayList<Integer>abuff,ArrayList<Integer>abuff_dot) throws FileNotFoundException {
        ...
        for (int i = 0; i < sign.size(); i++) {
            if (sign.get(i)==37){// %
                if (num==0){
                   long upper=Math.multiplyExact(abuff.get(i), abuff.get(i+1));
                   num=Math.floorDiv(upper,100);
                }else{
                    long upper=Math.multiplyExact(num,abuff.get(i+1));
                    num=Math.floorDiv(upper, 100);
                }
            }
        }// %
...
// ^=94  %=37 +=43 *=42 -=45 /=47 (->40  )->41 .=46
    ...
    private String calculate() throws IOException {
        ...
        long bodmas = bodmas(buffer,buffer_dot);
